...
public class AtBlueGoal extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        TrajectoryActionBuilder shoot2 = drive.actionBuilder(new Pose2d(-16, -54, Math.toRadians(270)))
                //from first row to shooting from medium range
                .strafeToLinearHeading(new Vector2d(-16,-17),Math.toRadians(225));

        TrajectoryActionBuilder collect2 = drive.actionBuilder(new Pose2d(-16, -17 , Math.toRadians(225)))
                //from medium range to second row
                .setTangent(0)
                .splineToLinearHeading(new Pose2d(7,-28, Math.toRadians(270)), Math.toRadians(270))
                .lineToYConstantHeading(-54);


        TrajectoryActionBuilder shoot3 = drive.actionBuilder(new Pose2d(7, -54, Math.toRadians(270)))
        ...
        TrajectoryActionBuilder collect3 = drive.actionBuilder(new Pose2d(-16, -17, Math.toRadians(225)))
        //from medium range to first row
                .setTangent(0)
                .splineToLinearHeading(new Pose2d(31,-30, Math.toRadians(270)), Math.toRadians(270))
                .lineToYConstantHeading(-54);

        TrajectoryActionBuilder shoot4 = drive.actionBuilder(new Pose2d(31, -54, Math.toRadians(270)))
                //from first row to shooting from medium range
                .setTangent(180)
                .splineToLinearHeading(new Pose2d(-16, -17 , Math.toRadians(225)), Math.toRadians(225));

        TrajectoryActionBuilder leave1 = drive.actionBuilder(new Pose2d(-16, -17 , Math.toRadians(225)))
                //leave the launch line
                .strafeToLinearHeading(new Vector2d(-42,-15), Math.toRadians(270));
